...
from libcity.pipeline import run_model_cross
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='COLAMetaLearning', help='the name of model')
    parser.add_argument('--dataset', type=str, default='porto', help='the name of target  dataset')  # 目标数据集
    parser.add_argument('--train_cities', type=str,nargs='+', default=["bj"])
    parser.add_argument('--domain_specific_params', nargs='+', default=["wte"])
    ...
    other_args = {key: val for key, val in dict_args.items() if key not in [
        'task', 'model', 'dataset', 'config_file', 'saved_model', 'train'] and
                  val is not None}
    run_model_cross(task=args.task, model_name=args.model, target_dataset_name=args.dataset,
