...
class ApproximateSimilarityEvaluator(AbstractEvaluator):
    ...
    def _init_path(self):
        ...
        self.knn_hit_path = './libcity/cache/{}/evaluate_cache/{}_knn_hit_{}_{}_{}.npy' \
            .format(self.exp_id, self.exp_id, self.model, self.dataset, self.d_model)
        self.database_pred_list, self.database_id_list, self.query_pred_list, self.query_id_list = None, None, None, \
            None

    def collect(self, batch):
        self.database_pred_list, self.database_id_list, self.query_pred_list, self.query_id_list = batch
    ...
    def evaluate(self):
        ...
        self.ground_truth_matrix = np.load(self.ground_truth_matrix_path)
    ...
    def evaluate_most_sim(self):
        ...
        if os.path.exists(self.euclidean_path):
            ...
            database_all = self.database_pred_list
            eul_res = euclidean_distances(self.query_pred_list, database_all)  # (a, b)
        ...
        for i in range(total_num):
            ...
            rank_index = rank_list.index(self.ground_truth_matrix[i][0])  # ground_truth's rank
            # rank_index is start from 0, so need plus 1
            rank += (rank_index + 1)
            rank_p += 1.0 / (rank_index + 1)
            for k in self.topk:
                if i in self.ground_truth_matrix[i][:k]:
                    hit[k] += 1
        ...
        for i in select_index:
            # query
            output.append([str(i) + '-query', self.query_id_list[i], self.query_wkt[str(self.query_id_list[i])],
                           self.ground_truth_matrix[i][0]])
            # detour
            ...
            for ind, d in enumerate(sorted_eul_index[i, 0:kmax]):
                ...
                if d == self.ground_truth_matrix[i][0]:
                    index_out += '-find'
                output.append([index_out, self.database_id_list[d],
                               self.database_wkt[str(self.database_id_list[d])], self.ground_truth_matrix[i][0]])
        output = pd.DataFrame(output, columns=['index', 'id', 'wkt', 'ground_truth_idx (' + self.measure_fn_name + ')'])
