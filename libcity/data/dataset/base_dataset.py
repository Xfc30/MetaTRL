...
class BaseDataset(AbstractDataset):
    def __init__(self, config):
        ...
        self.max_val_size = self.config.get('max_val_size', None)
        self.max_test_size = self.config.get('max_test_size', None)
        ...
        self.geo_struct_features_file = self.config.get('struct_features_file', None)
        ...
        if self.geo_struct_features_file is not None:
            self.geo_struct_features_file = self._load_struct_feature_file()
        ...
        if self.add_meta_gat:
            ...
            self.node_struct_features, self.node_shape_features, self.node_struct_fea_dim = self._load_geo_struct_feature(
            self.geo_struct_features_file)
        # self.node_features = np.concatenate((self.node_features, self.node_shape_features), axis=1)
            self.node_features = torch.cat([self.node_features, self.node_shape_features],dim=-1)
            self.node_fea_dim = self.node_features.shape[-1]
            self.logger.info("shape of node features after cat:{}".format(self.node_features.shape))
            ...
            self.node_features, self.node_fea_dim = self._load_geo_feature(self.geo_file)
            self.edge_index, self.loc_trans_prob = self._load_k_neighbors_and_trans_prob()
            self.node_struct_features = None
            self.node_struct_fea_dim = 0
            self.adj_mx_encoded = None
            self.hypergraph = None
        else:
    ...
    def _load_struct_feature_file(self):
        struct_features = pd.read_csv(self.data_path + self.geo_struct_features_file)
        self.geo_ids = list(struct_features['geo_id'])
        self.ind_to_geo_of_struct_feature_file = {}
        for index, geo_id in enumerate(self.geo_ids):
            self.ind_to_geo_of_struct_feature_file[index] = geo_id
    ...
    def _load_geo_struct_feature(self, geo_struct_features_info):
        struct_fea_path = self.data_path + '{}_struct_features.npy'.format(self.roadnetwork)
        if os.path.exists(struct_fea_path):
            geo_struct_features = np.load(struct_fea_path)
        else:
            useful = ['shape_diff', 'neighbor_mean_dist', 'neighbor_std_dist', 'neighbor_min_dist', 'neighbor_max_dist']
            geo_struct_features = geo_struct_features_info[useful]
            norm_dict = {'neighbor_mean_dist': 1, 'neighbor_std_dist': 2, 'neighbor_min_dist': 3,
                         'neighbor_max_dist': 4}
            for k, v in norm_dict.items():
                d = geo_struct_features[k]
                min_ = d.min()
                max_ = d.max()
                dnew = (d - min_) / (max_ - min_)
                geo_struct_features = geo_struct_features.drop(columns=k)  # drop报错
                geo_struct_features.insert(v, k, dnew)
            log_norm_dict = {'shape_diff': 1}
            # 对shape_diff归一化
            geo_struct_features[['lon_diff', 'lat_diff']] = geo_struct_features['shape_diff'].str.split(',',
                                                                                                        expand=True).astype(
                float)

            # 分别进行 min-max 归一化
            lon_min, lon_max = geo_struct_features['lon_diff'].min(), geo_struct_features['lon_diff'].max()
            lat_min, lat_max = geo_struct_features['lat_diff'].min(), geo_struct_features['lat_diff'].max()
            geo_struct_features['lon_diff_norm'] = (geo_struct_features['lon_diff'] - lon_min) / (
                    lon_max - lon_min + 1e-9)
            geo_struct_features['lat_diff_norm'] = (geo_struct_features['lat_diff'] - lat_min) / (
                    lat_max - lat_min + 1e-9)
            geo_struct_features = geo_struct_features.drop(columns=['lon_diff', 'lat_diff', 'shape_diff'])
            geo_struct_features = geo_struct_features.values  # (N,6)
            np.save(struct_fea_path, geo_struct_features)
        self.logger.info("node struct features: " + str(geo_struct_features.shape))
        node_struct_features = np.zeros((self.vocab.vocab_size, geo_struct_features.shape[1]))
        node_shape_features = np.zeros((self.vocab.vocab_size, 2))
        for ind in range(len(geo_struct_features)):
            geo_id = self.ind_to_geo_of_struct_feature_file[ind]
            encoded_geo_id = self.vocab.loc2index[geo_id]
            node_struct_features[encoded_geo_id] = geo_struct_features[ind]
            node_shape_features[encoded_geo_id] = geo_struct_features[ind, -2:]
        node_struct_features = torch.from_numpy(node_struct_features).float().to(self.device)
        node_shape_features = torch.from_numpy(node_shape_features).float().to(self.device)
        self.logger.info("node_struct_features: " + str(node_struct_features.shape))
        self.logger.info("node_shape_features: " + str(node_shape_features.shape))
        return node_struct_features, node_shape_features, node_shape_features.shape[1]
    ...
    def get_data_feature(self):
        data_feature = {'usr_num': self.usr_num, 'vocab_size': self.vocab_size, 'vocab': self.vocab,
                        ...
                        "node_struct_features": self.node_struct_features,
                        "node_struct_fea_dim": self.node_struct_fea_dim,
...
class TrajectoryProcessingDataset(Dataset):
    ...
    def __init__(self, data_name, data_type, vocab, seq_len=512,
                 add_cls=True, merge=True, min_freq=1, max_train_size=None, max_val_size=None, max_test_size=None):
        self.vocab = vocab
    ...
    #         self.all_traj_list = pickle.load(open(self.cache_path, 'rb'))
    #         self.all_temporal_mat_list = pickle.load(open(self.temporal_mat_path, 'rb'))
    #         self._logger.info('Load dataset from {}'.format(self.cache_path))
    #     else:
    #         origin_data_df = pd.read_csv(self.data_path, sep=';')
    #         self.all_traj_list, self.all_temporal_mat_list = self.data_processing(origin_data_df)
    #     # train_end, val_end, test_end = self.max_train_size, self.max_train_size + self.max_val_size, self.max_train_size + self.max_val_size + self.max_test_size
    #     if self.max_train_size is not None:
    #         self.traj_list = self.all_traj_list[:self.max_train_size]
    #         self.temporal_mat_list = self.all_temporal_mat_list[:self.max_train_size]
    #     else:
    #         self.max_train_size=0
    #     if self.max_val_size is not None:
    #         self.traj_list = self.all_traj_list[self.max_train_size:self.max_train_size+self.max_val_size]
    #         self.temporal_mat_list = self.all_temporal_mat_list[self.max_train_size:self.max_train_size+self.max_val_size]
    #     else:
    #         self.max_val_size=0
    #     if self.max_test_size is not None:
    #         self.traj_list = self.all_traj_list[self.max_train_size+self.max_val_size:self.max_train_size+self.max_val_size+self.max_test_size]
    #         self.temporal_mat_list = self.all_temporal_mat_list[self.max_train_size+self.max_val_size:self.max_train_size+self.max_val_size+self.max_test_size]
    ...
    def __getitem__(self, ind):
        if ind >= len(self.traj_list) or ind >= len(self.temporal_mat_list):
            print("Index Error")
