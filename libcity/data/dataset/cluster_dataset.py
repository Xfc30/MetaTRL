...
class DownStreamSubDataset(TrajectoryProcessingDataset):
    ...
    def data_processing(self, origin_data):
        ...
        for i in tqdm(range(sub_data.shape[0]), desc=self.data_path):
            ...
            # cal wkt str : 轨迹的每一个路段转为经纬度
            wkt_str = 'LINESTRING('
            for j in range(len(loc_list)):  # 第j个路段
                rid = loc_list[j]
...
def collate_unsuperv_down(data, max_len=None, vocab=None, add_cls=True):  # data: list of tuple (包含两个元素)
    batch_size = len(data)
