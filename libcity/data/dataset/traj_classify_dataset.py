...
class ClassifySubDataset(TrajectoryProcessingDataset):
    ...
    def data_processing(self, origin_data):
        ...
        for i in tqdm(range(sub_data.shape[0])):
            ...
            traj_fea = np.array([new_loc_list, tim_list, minutes, weeks, usr_list, vflag_list]).transpose((1, 0))  # traj_fea: (seq_length, feat_dim) [4][-1]
        ...
        # traj_list : data_nums * seq_length * feat_dim
...
def collate_superv_classify_usrid(data, max_len=None, vocab=None, add_cls=True):
    batch_size = len(data)
    # list of (seq_length, feat_dim) # features, temporal_mat 都是元组，元组的内容是 list of (seq_length, feat_dim), 最终 features是一个 batchsize * seq_length* feat_dim 的元组
    features, temporal_mat = zip(*data)
    ...
    for i in range(batch_size):
        ...
        X[i, :end, :] = features[i][:end, :]
        labels.append(features[i][-1][4]) # 标签的位置 参照第83行的代码， 原本是[4][-1]transpose之后就是[-1][4]
        batch_temporal_mat[i, :end, :end] = temporal_mat[i][:end, :end]
    X[:, :, 4] = vocab.pad_index
