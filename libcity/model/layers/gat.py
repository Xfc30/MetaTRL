...
class MetaGATLayer(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_meta_features, num_of_heads, concat=True,
                 activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, load_trans_prob=True):
        ...
        # todo 1. meta weight
        #      2. 更新门控
        self.meta_features_mlp = nn.Sequential(nn.Linear(num_meta_features, num_in_features), nn.Sigmoid(),
                                               nn.Linear(num_in_features, num_in_features))
        self.linear_proj_weight_source = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)
        self.linear_proj_weight_target = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)

    def forward(self, data):
        #
        # Step 1: Linear Projection + regularization
        #
        in_nodes_features, nodes_meta_features, edge_index, edge_prob = data  # unpack data edge_prob=(E, 1)
        num_of_nodes = in_nodes_features.shape[self.nodes_dim]
        assert edge_index.shape[0] == 2, f'Expected edge index with shape=(2,E) got {edge_index.shape}'
        ...
        # Note: for Cora features are already super sparse so it's questionable how much this actually helps
        in_nodes_features = self.dropout(in_nodes_features)
        meta_embs = self.meta_features_mlp(nodes_meta_features)

        # shape = (N, FIN) * (FIN, NH*FOUT) -> (N, NH, FOUT) where NH - number of heads, FOUT - num of output features
        # We project the input node features into NH independent output features (one for each attention head)
        nodes_features_proj = self.linear_proj(in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        nodes_features_proj = self.dropout(nodes_features_proj)  # in the official GAT imp they did dropout here as well
        scoring_fn_source = self.linear_proj_weight_source(meta_embs).view(-1, self.num_of_heads, self.num_out_features)
        scoring_fn_target = self.linear_proj_weight_target(meta_embs).view(-1, self.num_of_heads, self.num_out_features)
        if self.load_trans_prob:
            # shape = (E, 1) * (1, NH*FOUT) -> (E, NH, FOUT) where NH - number of heads, FOUT - num of output features
            trans_prob_proj = self.linear_proj_tran_prob(edge_prob).view(
                -1, self.num_of_heads, self.num_out_features)  # (E, NH, FOUT)
        ...
        # shape = (N, NH, FOUT) * (1, NH, FOUT) -> (N, NH, FOUT) -> (N, NH) because sum squeezes the last dimension
        # Optimization note: torch.sum() is as performant as .sum() in my experiments
        scores_source = (nodes_features_proj * scoring_fn_source).sum(dim=-1)
        scores_target = (nodes_features_proj * scoring_fn_target).sum(dim=-1)
        # todo 实现 metagat 只需要更改以上可学习参数, 改为根据元知识学习
        # We simply copy (lift) the scores for source/target nodes based on the edge index. Instead of preparing all
        # the possible combinations of scores we just prepare those that will actually be used and those are defined
        # by the edge index.
        # scores shape = (E, NH), nodes_features_proj_lifted shape = (E, NH, FOUT), E - number of edges in the graph
        ...
        #
        out_nodes_features = self.skip_concat_bias(in_nodes_features, out_nodes_features)
        return (out_nodes_features, nodes_meta_features, edge_index, edge_prob)
