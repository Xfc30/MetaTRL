...
from libcity.executor import BertBaseExecutor
from libcity.model import loss
from libcity.model.trajectory_embedding import MetaTRLLinearETA
from libcity.utils.utils import cul_batch_time_interval


class MetaTRLETAExecutor(BertBaseExecutor):
    ...
    def _load_pretrain_model(self):
        ...
        if isinstance(self.model, MetaTRLLinearETA):
            self.model.colads.cola.load_state_dict(pretrained_model.model.state_dict())
        else:
            raise ValueError('No COLA model in model!')
        if self.freeze:
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="Train epoch={}".format(
            ...
            predictions = self.model(x=X,padding_masks=padding_masks,batch_temporal_mat=batch_interval,graph_dict=graph_dict)  # (batch_size, 1)
